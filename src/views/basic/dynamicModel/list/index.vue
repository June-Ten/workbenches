              ...
              <el-table-column :prop="item.prop" :label="item.label" :align="item.align"
                      ...
                      <template slot-scope="scope">
                        <template v-if="scope.row.rowEdit">
                          ...
                          <template v-else-if="['rate','slider'].includes(child.jnpfKey)">
                            <el-input-number v-model="scope.row[child.prop]" placeholder="请输入"
                ...
                <template slot-scope="scope">
                  ...
                  <template v-else>
                    ...
                    <template v-if="['inputNumber','calculate'].includes(item.jnpfKey)">
                  ...
                  <template v-if="item.children&&item.children.length">
                    <template v-for="child in item.children">
                      <el-table-column v-if="child.jnpfKey=='relationForm'" :align="child.align"
                        ...
                        :width="child.width" :sortable="child.sortable?'custom':child.sortable">
                        <template slot-scope="scope">
                          <el-link :underline="false"
  ...
  methods: {
    ...
    */
    ...
    handleNodeClick(data, node) {
      ...
      let queryJson = {}
      if (this.columnData.treeDataSource === "organize" || data.parentId) {
      ...
      for (let i = 0; i < this.columnData.columnList.length; i++) {
        ...
        if (e.__config__.jnpfKey === 'date') {
          ...
          if (this.userInfo.organizeIdList instanceof Array && this.userInfo.organizeIdList.length > 0) {
            item[e.id] = e.multiple == true ? [this.userInfo.organizeIdList] : this.userInfo.organizeIdList
          } else {
            item[e.id] = []
          }
