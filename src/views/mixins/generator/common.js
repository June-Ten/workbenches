...
export default {
  ...
  methods: {
    ...
    async updateFields(webType) {
      if (webType == 4) {
        await getDataInterfaceInfo(this.dataForm.interfaceId).then(res => {
          this.viewFields = res.data.fieldJson ? JSON.parse(res.data.fieldJson) : [];
          this.dataForm.interfaceName = res.data.fullName
        })
      }
      if (!this.tables.length) {
        this.loading = false
