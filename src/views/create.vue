<template>
  <div class="list create-screen">
    ...
    <div class="create-box">
      ...
      <div class="create-scroll">
        <div class="content__box">
          <div class="content__item" :class="{ 'content--active': count == index }" v-for="(item, index) in list"
            @click="count = index" :key="index">
    ...
    <div class="create-content">
      <img :src="active.background_url" alt="">
...
export default {
  ...
  created() {
    this.form.category_value = this.$route.query.category;
  },
  mounted() {
    this.form.category_value = this.$route.query.category;
    this.getList()
  ...
  methods: {
    getList() {
      getList({
        category_value: this.form.category_value || 1,
        current: 1,
        size: 67
      }).then(res => {
        ...
        this.list.unshift({
          id: 0,
          title: '空白模板',
          backgroundUrl: `${window.location.origin}${window.location.pathname}img/bg.jpg`
        })
    ...
    handleSave() {
      if (this.count == 0) {
        addObj(this.form).then(res => {
          console.log(res, 'asdfwef')
          const id = res.data.data || 0;
          this.handleEdit({ id })
        })
      } else {
        let id;
        copyObj(this.active.Kvid).then(res => {
          id = res.data.data
          return updateObj({
            Kvid: id,
    ...
    handleEdit(item) {
      const routeUrl = window.location.origin + window.location.pathname + `#/build/?Kvid=` + item.id
