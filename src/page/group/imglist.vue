<template>
  <el-dialog title="图库" width="80%" :close-on-click-modal="false" :visible.sync="imgVisible">
    <div style="margin:0 auto;">
      <el-upload class="upload-demo" :on-success="onSuccess" :show-file-list="false" :action="url + '/visual/put-file'"
        multiple list-type="picture">
        <el-button size="small" icon="el-icon-upload" type="primary">点击上传</el-button>
      </el-upload>
    </div>
    <el-scrollbar class="imgList">
      <img :src="`${publicPath}${item.value}`" :style="styleName" @click="handleSetimg(item.value)"
...
export default {
  data() {
    return {
      ...
      imgOption: imgOption,
      imgTabs: [],
      publicPath: `${window.location.origin}${window.location.pathname}`
    }
  },
  computed: {
    styleName() {
