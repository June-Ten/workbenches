<template>
  <el-dialog title="选项设置" :close-on-click-modal="false" :visible.sync="visible"
    ...
    <JNPF-table :data="list" ref="dragTable" :hasNO="false" row-key="id" v-if="type==1">
      ...
      <el-table-column prop="filedName" label="字段">
        <template slot-scope="scope">
          <el-input v-model="scope.row.filedName" placeholder="请输入字段" />
        </template>
      </el-table-column>
    ...
    <el-table :data="list" ref="dragTable" :hasNO="false" v-else>
      ...
      <el-table-column prop="filedName" label="字段">
        <template slot-scope="scope">
          <el-autocomplete v-model="scope.row.filedName" :fetch-suggestions="querySearchAsync"
