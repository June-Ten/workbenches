        ...
        <JNPF-table :data="list" ref="dragTable" :hasNO="false" row-key="id">
          ...
          <el-table-column prop="field" label="名称" width="160px">
            <template slot-scope="scope">
              <el-input v-model="scope.row.fullName" placeholder="请输入名称" maxlength="50" clearable />
            </template>
          </el-table-column>
          <el-table-column prop="num" width="180px" label="数值" v-if="type==1">
            <template slot-scope="scope">
              <el-input v-model="scope.row.num" placeholder="请输入数值" maxlength="10" />
            </template>
          </el-table-column>
          <el-table-column prop="field" label="字段" width="140px" v-else>
            <template slot-scope="scope">
              <el-autocomplete v-model="scope.row.field" :fetch-suggestions="querySearchAsync"
                clearable placeholder="请输入字段" @select="handleSelect($event,scope.$index)"
                style="width:100%">
          ...
          <el-table-column prop="dataLength" width="200px">
            <template slot-scope="scope">
              <JnpfTreeSelect v-if="scope.row.linkType==1"
...
export default {
  components: { iconBox },
  props: ['menuList', 'appMenuList', 'showType', 'type', 'interfaceId'],
  data() {
    return {
      visible: false,
      list: [],
  ...
  methods: {
    ...
    confirm() {
      for (let i = 0; i < this.list.length; i++) {
        ...
        if (this.type == 2 && !element.field) return this.$message.warning('字段不能为空')
        if (this.type == 1 && !element.num) return this.$message.warning('数值不能为空')
    ...
    handleSelect(item, index) {
      this.list[index].field = item.defaultValue
    }
