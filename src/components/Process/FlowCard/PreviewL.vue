<script>
import { NodeUtils } from "./projectLife.js";
...
function createNormalCard(ctx, conf, h) {
  ...
  const isOsNode = afterTrue(NodeUtils.isOutsourceNode(conf), 'outsource')
  const isTrialNode = afterTrue(NodeUtils.isTrailNode(conf), 'trial')
  const isLgNode = afterTrue(NodeUtils.isLingguNode(conf), 'linggu')
  const isChangeNode = afterTrue(NodeUtils.isChangeNode(conf), 'change')
  const isFinishPNode = afterTrue(NodeUtils.isFinishPNode(conf), 'finishP')
...
function addEndNode(h) {
  ...
  return <section class="end-node"></section>;
}
