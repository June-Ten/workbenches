    ...
    <div class="main-board">
      <div class="search-box">
        ...
        <el-table :data="columnData.searchList" class="JNPF-common-table" ref="dragTableSearch"
          ...
          <el-table-column prop="id" label="字段" />
          ...
          <el-table-column prop="label" label="默认值" width="400" v-if="webType != 4">
            <template slot-scope="scope">
              <el-input v-model="scope.row.value" placeholder="默认值"
                v-if="textBoxList.includes(scope.row.jnpfKey)" />
              <num-range v-if="scope.row.jnpfKey==='inputNumber'" v-model="scope.row.value"
                :precision="scope.row.precision" clearable filterable></num-range>
...
import { noColumnShowList, noSearchList, useInputList, useDateList, useSelectList, dyOptionsList } from '@/components/Generator/generator/comConfig'
...
export default {
  ...
  components: { draggable, FormScript, uploadBox, CustomBtn, InterfaceDialog, Condition, ComplexHeader },
  data() {
    return {
      fieldJsonList: [],
      hasPage: false,
  ...
  created() {
    ...
    if (this.webType != 4) {
      ...
      this.groupFieldOptions = list.filter(o => o.id.indexOf('-') < 0)
  ...
  computed: {
    getSearchOptions() {
      if (this.columnData.type == 2 && this.columnData.treeRelation) this.searchOptions.filter(o => o.id !== this.columnData.treeRelation);
  ...
  methods: {
    ...
    */
    ...
    toggleType(val) {
      ...
      if (val == 2) this.onTreeRelationChange(this.columnData.treeRelation);
