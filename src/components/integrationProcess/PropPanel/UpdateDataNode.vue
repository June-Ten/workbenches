    ...
    <el-scrollbar class="config-scrollbar">
      <el-form label-position="top" :model="formConf" class="pd-10-20">
        <el-form-item>
          ...
          <el-select v-model="formConf.flowId" placeholder="请选择流程" filterable
        ...
        <el-form-item v-if="integrateType == 2">
...
export default {
  ...
  methods: {
    ...
    addItem(index) {
      ...
      if (this.integrateType == 2) emptyChildItem.symbol = '=='
      this.ruleList[index].groups.push(emptyChildItem);
    },
    delItem(index, childIndex) {
      if (this.integrateType == 2 && this.ruleList.length <= 1 && this.ruleList[0].groups.length <= 1) {
    ...
    addGroup() {
      let emptyItem = JSON.parse(JSON.stringify(this.emptyItem))
      if (this.integrateType == 2) emptyItem.groups[0].symbol = '=='
      this.ruleList.push(emptyItem);
    },
    delGroup(index) {
      if (this.integrateType == 2 && this.ruleList.length <= 1) return this.$message.error('必须保留一个条件');
    ...
    onFieldChange(item, val, index, childIndex) {
      ...
      if (this.integrateType == 2) item.symbol = '=='
