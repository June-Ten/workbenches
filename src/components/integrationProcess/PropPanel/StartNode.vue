<template>
  <section class="approver-pane">
    <el-tabs v-model="activeKey" class="pane-tabs">
      <el-tab-pane name="1" label="表单配置"></el-tab-pane>
      ...
      <el-form label-position="top" :model="formConf" class="pd-10-20" v-show="activeKey === '1'">
        <template v-if="integrateType == 1">
          <el-form-item>
            ...
            <FlowFormModal v-model="formConf.formId" :title="formConf.formName" :enableFlow="0"
...
export default {
  name: 'StartNode',
  components: { FlowFormModal, Condition, MsgDialog, Detail, vcrontab },
  props: {
    formConf: {
      type: Object,
      default: () => { }
